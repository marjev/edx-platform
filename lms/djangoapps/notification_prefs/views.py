...
class UsernameCipher(object):
    ...
    def encrypt(username):
        initialization_vector = os.urandom(UsernameCipher.aes_block_size_bytes)
    ...
    def decrypt(token):
        ...
        if len(base64_decoded) < UsernameCipher.aes_block_size_bytes:
            raise UsernameDecryptionException("initialization_vector")

        initialization_vector = base64_decoded[:UsernameCipher.aes_block_size_bytes]
        aes_encrypted = base64_decoded[UsernameCipher.aes_block_size_bytes:]
        aes_cipher = UsernameCipher._get_aes_cipher(initialization_vector)
