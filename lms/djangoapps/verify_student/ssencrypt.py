...
def rsa_encrypt(data, rsa_pub_key_str):
    ...
    """
    if rsa_pub_key_str.startswith(b'-----'):
        key = serialization.load_pem_public_key(rsa_pub_key_str, backend=default_backend())
    else:
        key = serialization.load_ssh_public_key(rsa_pub_key_str, backend=default_backend())
...
def rsa_decrypt(data, rsa_priv_key_str):
    ...
    """
    key = serialization.load_pem_private_key(rsa_priv_key_str, password=None, backend=default_backend())
